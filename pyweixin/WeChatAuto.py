...
from .Uielements import (Main_window, SideBar, Independent_window, Buttons,
                         Edits, Texts, TabItems, Lists, Panes, Windows, CheckBoxes, MenuItems, Menus, Groups, Customs,
                         ListItems)
...
class AutoReply():
    ...
    @staticmethod
    def auto_reply_to_friend(dialog_window: WindowSpecification, duration: str, callback: Callable[[str, list], str],
                             save_file: bool = False, target_folder: str = None,
                             close_dialog_window: bool = True) -> dict:
        '''
                    ...
                    if newMessage.class_name() == 'mmui::ChatTextItemView':
                        ...
                        if not is_my_bubble:
                            ...
                            if reply_content is not None:
                                ...
                                win32gui.SendMessage(dialog_window.handle, win32con.WM_SYSCOMMAND, win32con.SC_MINIMIZE,
                                                     0)
                    if Tools.is_voice_message(newMessage):
                        voice_count += 1
                        dialog_window.restore()
                        is_my_bubble = Tools.is_my_bubble(dialog_window, newMessage, input_edit)
                        if not is_my_bubble:
                            voice_text = Tools.convert_voice_message_to_text(dialog_window, chatList, newMessage)
                            if voice_text:
                                texts.append(f'[语音转文字]{voice_text}')
                                contexts = [listitem.window_text() for listitem in chatList.children(control_type='ListItem')]
                                reply_content = callback(voice_text, contexts)
                                if reply_content is not None:
                                    input_edit.set_text(reply_content)
                                    pyautogui.hotkey('alt', 's')
                                    win32gui.SendMessage(dialog_window.handle, win32con.WM_SYSCOMMAND,
                                                         win32con.SC_MINIMIZE, 0)
                    if newMessage.class_name() == 'mmui::ChatBubbleItemView' and newMessage.window_text()[
                        :2] == '[链接]':  #
                        link_count += 1
        ...
        details = {'新消息总数': total, '文本数量': len(texts), '文件数量': len(files), '图片数量': image_count,
                   '视频数量': video_count, '链接数量': link_count, '语音数量': voice_count, '文本内容': texts}
        return details
...
class Collections():
    ...
    @staticmethod
    def cardLink_to_url(number: int, delete: bool = False, delay: float = 0.5, is_maximize: bool = None,
                        close_weixin: bool = None) -> dict[str, str]:
        ...
        selected_item = [listitem for listitem in link_list.children(control_type='ListItem') if
                         listitem.has_keyboard_focus() and listitem.window_text() != ''][0]
    ...
    @staticmethod
    def collect_offAcc_articles(name: str, number: int, delay: float = 0.3, is_maximize: bool = None,
        ...
        clicked = []
        collected_num = 0
        seperate_window = Navigator.open_seperate_dialog_window(friend=name, is_maximize=is_maximize,
        ...
        while collected_num < number:
            visible_texts = [child for child in container.children(control_type='Text') if
                             child.is_visible() and timestamp_pattern.search(
                                 child.window_text()) and child.element_info.runtime_id not in clicked]
            if visible_texts:
...
class Contacts():
    ...
    @staticmethod
    def get_friends_detail(is_maximize: bool = None, close_weixin: bool = None, is_json: bool = False,
                           save_avatar: bool = False, avatar_folder: str = None, overwrite_avatar: bool = True,
                           mark_stale_avatar: bool = True) -> (list[dict] | str):
        '''
        该方法用来获取通讯录内好友信息
        Args:
            ...
            close_weixin:任务结束后是否关闭微信，默认关闭
            is_json:是否以json格式返回
            save_avatar:获取好友信息时是否同步保存头像，默认False
            avatar_folder:头像保存目录，save_avatar=True时生效
            overwrite_avatar:头像文件已存在时是否覆盖
            mark_stale_avatar:是否将不在本次好友列表中的索引项标记为stale
        Returns:
        ...
        def get_specific_info():
            ...
            info = {'昵称': nickname, '微信号': wx_number, '地区': region, '备注': remark, '电话': phonenumber,
                    '标签': tag, '描述': descrption, '朋友权限': permission, '共同群聊': f'{common_group_num}',
                    '个性签名': signature, '来源': source}
            return info

        def save_avatar_from_contact_profile(info: dict):
            display_name = info.get('备注') if info.get('备注') and info.get('备注') != '无' else info.get('昵称')
            if not display_name:
                return None, None
            ...
            safe_name = Contacts._safe_avatar_text(display_name) or 'unknown'
            safe_id = Contacts._safe_avatar_text(
                wx_number if wx_number and wx_number != '无' else display_name) or 'unknown_id'
            filename = f'{safe_name}_{safe_id}.png'
            save_path = os.path.join(avatar_folder, filename)
            if overwrite_avatar or (not os.path.isfile(save_path)):
            ...
            avatar_entry = {
                ...
                'nickname': info.get('昵称', '无'),
                'wechat_id': wx_number,
                'friend_input': display_name,
            ...
            return unique_key, avatar_entry
        ...
        if close_weixin is None:
            close_weixin = GlobalConfig.close_weixin
        if save_avatar:
            if avatar_folder is None:
                avatar_folder = os.path.join(os.getcwd(), 'friend_avatars')
            os.makedirs(avatar_folder, exist_ok=True)
            avatar_index_path = os.path.join(avatar_folder, 'index.json')
            avatar_index = Contacts._load_avatar_index(avatar_index_path)
            avatar_map = {item.get('unique_key'): item for item in avatar_index.get('avatars', []) if
                          isinstance(item, dict) and item.get('unique_key')}
            active_avatar_keys = set()
        labels = {'微信号：', '昵称：', '地区：', '备注', '共同群聊', '个性签名', '来源', '电话', '描述', '标签',
                  '朋友权限', '朋友圈'}  #联系人分区的标签
        friends_detail = []
        #通讯录列表
        ...
        contact_item = main_window.child_window(control_type='ListItem', title_re=r'联系人\d+',
        ...
        if contact_item.exists(timeout=0.1):
            ...
            if save_avatar:
                key, entry = save_avatar_from_contact_profile(info)
                if key:
                    active_avatar_keys.add(key)
                if entry:
                    avatar_map[key] = entry
                    info['头像路径'] = entry['saved_path']
        ...
        if save_avatar:
            now = time.strftime('%Y-%m-%d %H:%M:%S')
            avatars = list(avatar_map.values())
            if mark_stale_avatar:
                for item in avatars:
                    if item.get('unique_key') in active_avatar_keys:
                        item['stale'] = False
                        item['last_seen_at'] = now
                    else:
                        item['stale'] = True
            avatar_index['avatars'] = avatars
            Contacts._save_avatar_index(avatar_index_path, avatar_index)
        if is_json:
    ...
    @staticmethod
    def get_wecom_friends_detail(is_maximize: bool = None, close_weixin: bool = None, is_json: bool = False) -> (
            list[dict] | str):
        '''
        ...
        #企业微信联系人分区
        Tools.collapse_contacts(main_window, contact_list)
        wecom_item = main_window.child_window(control_type='ListItem', title_re=r'企业微信联系人\d+',
                                              class_name="mmui::ContactsCellGroupView")
    ...
    @staticmethod
    def get_serAcc_info(is_maximize: bool = None, close_weixin: bool = None, is_json: bool = False) -> (
            list[dict] | str):
        '''
        该方法用来获取通讯录内服务号信息
        Args:
        ...
        #企业微信联系人分区
        Tools.collapse_contacts(main_window, contact_list)
        service_item = main_window.child_window(control_type='ListItem', title_re=r'服务号\d+',
                                                class_name="mmui::ContactsCellGroupView")
    ...
    @staticmethod
    def get_offAcc_info(is_maximize: bool = None, close_weixin: bool = None, is_json: bool = False) -> (
            list[dict] | str):
        '''
        该方法用来获取通讯录内公众号信息
        Args:
        ...
        #企业微信联系人分区
        Tools.collapse_contacts(main_window, contact_list)
        official_item = main_window.child_window(control_type='ListItem', title_re=r'公众号\d+',
                                                 class_name="mmui::ContactsCellGroupView")
    ...
    def get_groups_info(is_maximize: bool = None, close_weixin: bool = None) -> list[str]:
        ...
        if check_all_buttons:
            ...
            for _ in range(total + 1):
                #获取灰色的被选中的listitem记录
                focused_item = [listitem for listitem in search_results.children(control_type='ListItem',
    ...
    def get_friend_profile(friend: str, search_pages: int = None, is_maximize: bool = None, close_weixin: bool = None):
        ...
        no_need_labels = ['企业', '昵称：', '备注', '实名', '职务', '员工状态', '朋友圈', '工作时间',
                          '在线时间', '地址', '发消息', '语音聊天', '视频聊天', '企业信息', '来自', '企业微信']
        profile_pane, main_window = Navigator.open_friend_profile(friend=friend, search_pages=search_pages,
    ...
    @staticmethod
    def _safe_avatar_text(text: str) -> str:
        '''将文本转换为可用于文件名的形式'''
    ...
    @staticmethod
    def _load_avatar_index(index_path: str) -> dict:
        '''读取头像索引文件，不存在时返回默认结构'''
        default_index = {'version': 1, 'updated_at': '', 'avatars': []}
        ...
        try:
            ...
            if not isinstance(index_data, dict):
                return default_index
            if not isinstance(index_data.get('avatars'), list):
                index_data['avatars'] = []
            index_data.setdefault('version', 1)
            index_data.setdefault('updated_at', '')
            return index_data
        except Exception:
            return default_index

    @staticmethod
    def _save_avatar_index(index_path: str, index_data: dict) -> None:
        '''写入头像索引文件'''
        index_data['updated_at'] = time.strftime('%Y-%m-%d %H:%M:%S')
    ...
    def save_friend_avatar(friend: str, target_folder: str = None, search_pages: int = None, is_maximize: bool = None,
        ...
        Args:
            friend:好友备注或昵称
            target_folder:头像保存目录，默认当前工作目录下friend_avatars
            search_pages:在会话列表中查找好友时滚动列表的次数
            is_maximize:微信界面是否全屏
            close_weixin:任务结束后是否关闭微信
            overwrite:是否覆盖同名头像，默认True
        Returns:
            result:头像保存结果与索引信息
        '''
        if is_maximize is None:
        ...
        if target_folder is None:
            target_folder = os.path.join(os.getcwd(), 'friend_avatars')
        os.makedirs(target_folder, exist_ok=True)
        index_path = os.path.join(target_folder, 'index.json')
        profile_pane, main_window = Navigator.open_friend_profile(friend=friend, is_maximize=is_maximize,
                                                                  search_pages=search_pages)
        try:
            texts = [item.window_text() for item in profile_pane.descendants(control_type='Text')]
            nickname = texts[0] if texts else friend
            wx_number = '无'
            remark = '无'
            if '微信号：' in texts:
                wx_number = texts[texts.index('微信号：') + 1]
            ...
            display_name = next((name for name in [remark, nickname, friend] if name and name != '无'), 'unknown')
            unique_id = wx_number if wx_number and wx_number != '无' else display_name
            safe_name = Contacts._safe_avatar_text(display_name) or 'unknown'
            safe_id = Contacts._safe_avatar_text(unique_id) or 'unknown_id'
            filename = f'{safe_name}_{safe_id}.png'
            ...
            unique_key = wx_number if wx_number and wx_number != '无' else f'friend:{display_name}'
            index_data = Contacts._load_avatar_index(index_path)
            now = time.strftime('%Y-%m-%d %H:%M:%S')
            entry = {
                'unique_key': unique_key,
                'remark': remark,
            ...
            index_data['avatars'] = avatars
            Contacts._save_avatar_index(index_path, index_data)
            result = {'friend': friend, 'saved_path': save_path, 'unique_key': unique_key, 'sha256': sha256}
            return result
        finally:
    ...
    @staticmethod
    def sync_friend_avatars(target_folder: str = None, search_pages: int = None, is_maximize: bool = None,
                            close_weixin: bool = None, overwrite: bool = True, mark_stale: bool = True,
                            is_json: bool = False) -> (list[dict] | str):
        '''
        按最新好友列表同步头像，支持更新索引与标记失效好友
        Args:
            target_folder:头像保存目录，默认当前工作目录下friend_avatars
            search_pages:在会话列表中查找好友时滚动列表的次数
            is_maximize:微信界面是否全屏
            close_weixin:任务结束后是否关闭微信
            overwrite:是否覆盖已存在头像文件，默认True(每次刷新时更新)
            mark_stale:是否将不在最新好友列表中的旧记录标记为stale，默认True
            is_json:是否返回json格式
        Returns:
            results:每个好友头像同步结果
        '''
        ...
        friends_detail = Contacts.get_friends_detail(is_maximize=is_maximize, close_weixin=True, is_json=False)
        results = []
        active_keys = set()
        for info in friends_detail:
            friend_name = info.get('备注') if info.get('备注') and info.get('备注') != '无' else info.get('昵称')
            if not friend_name:
                continue
            try:
                result = Contacts.save_friend_avatar(
                    friend=friend_name,
                ...
                active_keys.add(result.get('unique_key'))
            except Exception as e:
                results.append({'friend': friend_name, 'error': str(e)})

        index_data = Contacts._load_avatar_index(index_path)
        if mark_stale:
            now = time.strftime('%Y-%m-%d %H:%M:%S')
            for item in index_data.get('avatars', []):
                if item.get('unique_key') not in active_keys:
                    item['stale'] = True
                    item['last_seen_at'] = item.get('last_seen_at', '')
                else:
                    item['stale'] = False
                    item['last_seen_at'] = now
            Contacts._save_avatar_index(index_path, index_data)
    ...
    def get_recent_groups(is_maximize: bool = None, close_weixin: bool = None) -> list[tuple[str]]:
        ...
        if not recent_group.exists(timeout=0.1):
            ...
            listitems = contacts_manage_list.children(control_type='ListItem',
                                                      class_name="mmui::ContactsManagerControlSessionCell")
...
class FriendSettings():
    ...
    @staticmethod
    def mute_notification(friend: str, mute: int = 0, fold_chat: int = 0, search_pages: int = None,
                          is_maximize: bool = None, close_weixin: bool = None):
    ...
    @staticmethod
    def pin_chat(friend: str, state: int = 0, search_pages: int = None, is_maximize: bool = None,
                 close_weixin: bool = None):
        '''该方法用来将好友聊天置顶
        Args:
            friend:好友备注
    ...
    @staticmethod
    def change_privacy(friend: str, chat_privacy: int = 1, sns_privacy: int = 0, search_pages: int = None,
                       is_maximize: bool = None, close_weixin: bool = None):
        '''该方法用来设置好友权限
        Args:
            friend:好友备注
    ...
    @staticmethod
    def star_friend(friend: str, state: int = 1, search_pages: int = None, is_maximize: bool = None,
                    close_weixin: bool = None):
    ...
    @staticmethod
    def delete_friend(friend: str, clear_chat_history: int = 1, search_pages: int = None, is_maximize: bool = None,
                      close_weixin: bool = None):
        '''该方法用来删除好友
        Args:
            friend:好友备注
    ...
    @staticmethod
    def add_to_blacklist(friend: str, state: int = 0, search_pages: int = None, is_maximize: bool = None,
                         close_weixin: bool = None):
    ...
    @staticmethod
    def change_remark(friend: str, remark: str, description: str = None, phoneNum: str = None,
                      clear_phoneNum: bool = False, search_pages: int = None, is_maximize: bool = None,
                      close_weixin: bool = None):
    ...
    @staticmethod
    def get_common_groups(friend: str, search_pages: int = None, is_maximize: bool = None, close_weixin: bool = None) -> \
    list[str]:
        '''
        该方法用来获取我与某些好友加入的所有共同群聊名称
        Args:
...
class Files():
    @staticmethod
    def send_files_to_friend(friend: str, files: list[str], with_messages: bool = False, messages: list = [str],
                             messages_first: bool = False,
                             send_delay: float = None, clear: bool = None, is_maximize: bool = None,
    ...
    def send_files_to_friends(friends: list[str], files_list: list[list[str]],
                              with_messages: bool = False, messages_list: list[list[str]] = [],
                              messages_first: bool = False,
                              clear: bool = None, send_delay: float = None, is_maximize: bool = None,
                              close_weixin: bool = None) -> None:
        '''
        该方法用于给多个好友或群聊发送文件
        Args:
    ...
    @staticmethod
    def save_chatfiles(friend: str, number: int, target_folder: str = None, is_maximize: bool = None,
                       close_weixin: bool = None):
        ...
        labels = [listitem.window_text() for listitem in
                  fileList.children(control_type='ListItem', class_name='mmui::FileListCell')]
        ...
        while len(labels) < number:
            ...
            texts = [listitem.window_text() for listitem in
                     fileList.children(control_type='ListItem', class_name='mmui::FileListCell')]
    ...
    def export_recent_files(target_folder: str = None, is_maximize: bool = None, close_weixin: bool = None):
        ...
        labels = [label for label in labels if
                  ('未下载' not in label) or ('已过期' not in label) or ('发送中断' not in label)]
        for label in labels:
            filename, timestamp = extract_info(label)
            filepath = os.path.join(chatfile_folder, timestamp, filename)
            if os.path.exists(filepath):
    ...
    def export_videos(year: str = time.strftime('%Y'), month: str = None, target_folder: str = None) -> list[str]:
        ...
        for folder_path in filtered_folders:  #遍历筛选后的每个文件夹
            ...
            exported_videos = [os.path.join(folder_path, filename) for filename in os.listdir(folder_path) if
...
class Settings():
    ...
    @staticmethod
    def notification_alert(alert_map: dict = {'newMessage': True, 'Call': True, 'Moments': True, 'Game': True,
...
class Moments():
    ...
    @staticmethod
    def dump_recent_posts(recent: Literal['Today', 'Yesterday', 'Week', 'Month'] = 'Today', number: int = None,
        ...
        not_contents = ['mmui::TimelineCommentCell', 'mmui::TimelineCell',
                        'mmui::TimelineAdGridImageCell']  #评论区，余下x条,广告,这三种不需要
        moments_window = Navigator.open_moments(is_maximize=is_maximize, close_weixin=close_weixin)
        ...
        if moments_list.children(control_type='ListItem'):
            while True:
                listitems = [listitem for listitem in moments_list.children(control_type='ListItem') if
                             listitem.class_name() not in not_contents]
    ...
    def like_posts(recent: Literal['Today', 'Yesterday', 'Week', 'Month'] = 'Today', number: int = None,
                   callback: Callable[[str], str] = None, is_maximize: bool = None, close_weixin: bool = None) -> list[
        dict]:
        '''
        ...
        def comment(content_listitem: ListItemWrapper, comment_listitem: ListItemWrapper, content: str):
            #评论操作
            mouse.move(coords=center_point)
            ellipsis_area = (content_listitem.rectangle().right - 44,
                             content_listitem.rectangle().bottom - 15)  #省略号按钮所处位置
        ...
        if moments_list.children(control_type='ListItem'):
            while True:
                moments_list.type_keys('{DOWN}', pause=0.1)
                selected = [listitem for listitem in moments_list.children(control_type='ListItem') if
                            listitem.has_keyboard_focus()]
    ...
    @staticmethod
    def dump_friend_posts(friend: str, number: int, save_detail: bool = False, target_folder: str = None,
                          search_pages: int = None, is_maximize: bool = None, close_weixin: bool = None) -> list[dict]:
        '''
        ...
        def save_media(sns_detail_list: ListViewWrapper, photo_num: int, video_num: int, detail_folder: str,
                       content: str):
        ...
        moments_window = Navigator.open_friend_moments(friend=friend, is_maximize=is_maximize,
                                                       close_weixin=close_weixin, search_pages=search_pages)
        ...
        contents = [listitem for listitem in moments_list.children(control_type='ListItem') if
                    listitem.class_name() not in not_contents]
        if contents:
            while True:
    ...
    @staticmethod
    def like_friend_posts(friend: str, number: int, callback: Callable[[str], str] = None, is_maximize: bool = None,
                          close_weixin: bool = None) -> list[dict]:
        '''
...
class Messages():
    ...
    @staticmethod
    def message_chain(group: str, content: str = None, theme: str = None, example: str = None, description: str = None,
    ...
    def check_new_messages(is_maximize: bool = None, search_pages: int = None, close_weixin: bool = None):
        ...
        if new_message_num:
            new_messages = []
            new_message_dict = scan_for_new_messages(main_window=main_window, is_maximize=is_maximize,
                                                     close_weixin=False)
    ...
    def dump_recent_sessions(recent: Literal['Today', 'Yesterday', 'Week', 'Month', 'Year'] = 'Today',
                             chat_only: bool = False, is_maximize: bool = None, close_weixin: bool = None) -> list[
        tuple]:
        '''
    ...
    @staticmethod
    def pull_messages(friend: str, number: int, search_pages: int = None, is_maximize: bool = None,
                      close_weixin: bool = None) -> list[str]:
        '''
        ...
        if not chat_list.exists(timeout=0.1):
            ...
            while len(messages) < number:
                ...
                selected = [listitem for listitem in chat_list.children(control_type='ListItem') if
                            listitem.has_keyboard_focus()]
    ...
    @staticmethod
    def dump_chat_history(friend: str, number: int, capture_alia: bool = False, alias_folder: str = None,
                          search_pages: int = None, is_maximize: bool = None, close_weixin: bool = None) -> tuple[
        list, list]:
        ...
        chat_history_window = Navigator.open_chat_history(friend=friend, is_maximize=is_maximize,
                                                          close_weixin=close_weixin, search_pages=search_pages)
        ...
        while len(messages) < number:
            ...
            selected = [listitem for listitem in chat_history_list.children(control_type='ListItem') if
                        listitem.has_keyboard_focus()]
            if selected:
        ...
        chat_history_window.close()
        messages = messages[:number]
        timestamps = [timestamp_pattern.search(message).group(0) if timestamp_pattern.search(
            message) else '微信红包或转账(时间戳为图片非文本无法获取)' for message in messages]
...
class Monitor():
    ...
    @staticmethod
    def listen_on_chat(dialog_window: WindowSpecification, duration: str, save_file: bool = False,
                       save_photo: bool = False, target_folder: str = None, close_dialog_window: bool = True) -> dict:
        '''
        ...
        def save_photos(chatList: ListViewWrapper):
            ...
            while saved_image_count != image_count:
                selected = [listitem for listitem in
                            chatList.children(control_type='ListItem', class_name='mmui::ChatBubbleReferItemView',
                                              title='图片')
                            if listitem.has_keyboard_focus()]
                if selected:
                    ...
                    if image_ids[-1] == unique_id:
                        ...
                        center_y = rectangle.mid_point().y
                        mouse.right_click(
                            coords=(side_x, center_y))  #不要直接right_click_input(),x方向应该是chatList左边界靠右一些,y方向是中心
    ...
    def grab_red_packet(dialog_window: WindowSpecification, duration: str, close_dialog_window: bool = True) -> int:
        ...
        red_envelop_view = dialog_window.child_window(class_name='mmui::PayRedEnvelopeInfoView', title='',
                                                      control_type='Group')  #微信红包点击后弹出的界面
