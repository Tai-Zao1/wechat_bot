...
class Regex_Patterns():
    ...
    def __init__(self):
        ...
        self.Chafile_Timestamp_pattern = re.compile(
            r'(\d{4}年\d{1,2}月\d{1,2}日|\d{1,2}月\d{1,2}日|昨天|星期\w|\d{1,2}:\d{2})')  # 微信聊天文件时间戳
        self.Snsdetail_Timestamp_pattern = re.compile(
            r'(?<=\s)\d{4}年\d{1,2}月\d{1,2}日\s\d{1,2}:\d{2}|\d{1,2}月\d{1,2}日\s\d{1,2}:\d{2}|昨天\s\d{1,2}:\d{2}|星期\w\s\d{1,2}:\d{2}|\d{1,2}:\d{2}\s$')  # 微信好友朋友圈主页内的时间戳
        self.Chathistory_Timestamp_pattern = re.compile(
            r'(?<=\s)(\d{4}年\d{1,2}月\d{1,2}日\s\d{2}:\d{2}|\d{1,2}月\d{1,2}日\s\d{2}:\d{2}|\d{2}:\d{2}|昨天\s\d{2}:\d{2}|星期\w\s\d{2}:\d{2})$')  # 聊天记录界面内的时间戳
        self.Session_Timestamp_pattern = re.compile(
            r'(?<=\s)(\d{4}/\d{1,2}/\d{1,2}|\d{1,2}/\d{1,2}|\d{2}:\d{2}|昨天 \d{2}:\d{2}|星期\w)$')  # 主界面左侧会话列表内的时间戳
        ...
        self.Article_Timestamp_pattern = re.compile(
            r'(\d{4}年\d{1,2}月\d{1,2}日|\d{1,2}月\d{1,2}日|昨天|星期\w|今天)')  # 公众号文章的时间戳
        self.Voice_pattern = re.compile(r'^语音\d+[\"“”]?秒')
...
def auto_reply_to_friend_decorator(duration: str, friend: str, search_pages: int = 5, is_maximize: bool = False,
        ...
        def wrapper():
            ...
            while time.time() < end_timestamp:
                ...
                if runtime_id != initial_runtime_id:
                    is_my_bubble = Tools.is_my_bubble(main_window, newMessage, edit_area)
                    if not is_my_bubble:
                        message_content = newMessage.window_text()
                        if Tools.is_voice_message(newMessage):
                            voice_text = Tools.convert_voice_message_to_text(main_window, chatList, newMessage)
                            if voice_text:
                                message_content = voice_text
                        reply_content = reply_func(message_content)
...
def scan_for_new_messages(main_window: WindowSpecification = None, delay: float = 0.3, is_maximize: bool = None,
                          close_weixin: bool = None) -> dict:
    '''
    ...
    def traverse_messsage_list(listItems):
        ...
        newMessageTips = [listItem.window_text() for listItem in listItems if
                          listItem.window_text() not in newMessageSenders]
