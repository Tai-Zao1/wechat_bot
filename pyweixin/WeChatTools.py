...
class Tools():
    ...
    @staticmethod
    def is_voice_message(listitem: ListItemWrapper) -> bool:
        '''判断是否为微信4.1的语音消息项'''
        return listitem.class_name() == 'mmui::ChatVoiceItemView'

    @staticmethod
    def extract_voice_text(window_text: str) -> str | None:
        '''从语音消息标题中提取转写结果'''
        if not window_text or not window_text.startswith('语音'):
            return None
        normalized = window_text.replace('"', '').replace('“', '').replace('”', '')
        matched = Regex_Patterns.Voice_pattern.match(normalized)
        if not matched:
            return None
        transcription = normalized[matched.end():].strip()
        if transcription.startswith('未播放'):
            transcription = transcription[3:].strip()
        return transcription or None

    @staticmethod
    def _find_listitem_by_runtime_id(chat_list: ListViewWrapper, runtime_id) -> ListItemWrapper | None:
        '''根据runtime_id回查消息列表项'''
        for child in chat_list.children(control_type='ListItem'):
            if child.element_info.runtime_id == runtime_id:
                return child
        return None

    @staticmethod
    def convert_voice_message_to_text(main_window: WindowSpecification, chat_list: ListViewWrapper,
                                      listitem: ListItemWrapper, timeout: float = 8.0,
                                      interval: float = 0.3) -> str | None:
        '''右键语音消息执行转为文字,并等待转写结果'''
        transcription = Tools.extract_voice_text(listitem.window_text())
        if transcription:
            return transcription
        runtime_id = listitem.element_info.runtime_id
        rect = listitem.rectangle()
        mouse.right_click(coords=(rect.left + 100, rect.mid_point().y))
        convert_menu_item = main_window.child_window(**MenuItems.ConvertVoiceToTextMenuItem)
        if not convert_menu_item.exists(timeout=0.5):
            pyautogui.press('esc')
            return None
        convert_menu_item.click_input()
        end_timestamp = time.time() + timeout
        while time.time() < end_timestamp:
            current_item = Tools._find_listitem_by_runtime_id(chat_list, runtime_id)
            if current_item is not None:
                transcription = Tools.extract_voice_text(current_item.window_text())
                if transcription:
                    return transcription
            time.sleep(interval)
        return None
