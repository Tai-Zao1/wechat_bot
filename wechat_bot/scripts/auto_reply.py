...
try:
    ...
    from wechat_bot.runtime import hold_wechat_ui
...
def main() -> int:
    ...
    if chat_mode not in {"api", "local", "rules"}:
        chat_mode = "api"
    log("聊天模式", chat_mode)
    reply_service = AutoReplyService(
        ReplyServiceConfig(
            mode=chat_mode,
            bailian_app_id=str(args.bailian_app_id or "").strip(),
            bailian_api_key=str(args.bailian_api_key or "").strip(),
            bailian_system_prompt=str(args.bailian_system or "").strip() or DEFAULT_BAILIAN_SYSTEM_PROMPT,
            bailian_endpoint=str(args.bailian_endpoint or DEFAULT_BAILIAN_ENDPOINT).strip(),
            bailian_timeout=float(args.bailian_timeout or 20.0),
        ),
        logger=log,
    )
    reply_service.prime_clients()
    ...
    try:
        self_sent_cache_mtime_ns = self_sent_file.stat().st_mtime_ns
    except Exception:
        self_sent_cache_mtime_ns = 0
    ...
    def scan_for_new_messages(
        ...
        def traverse_message_list(list_items: list[Any]) -> tuple[list[str], list[int]]:
            filtered = [
                item
    ...
    def rect_to_text(rect: Any) -> str:
        try:
            return f"({int(rect.left)},{int(rect.top)},{int(rect.right)},{int(rect.bottom)})"
        except Exception:
            return "(unknown)"

    def log_item_geometry(friend: str, item: Any, *, source: str, stage: str, verdict: str, text: str = "") -> None:
    ...
    def remember_recent_sent_text(friend: str, text: str) -> None:
        ...
        try:
            save_self_sent_cache(self_sent_file, self_sent_cache)
            nonlocal self_sent_cache_mtime_ns
            try:
                self_sent_cache_mtime_ns = self_sent_file.stat().st_mtime_ns
            except Exception:
                self_sent_cache_mtime_ns = 0
    ...
    def refresh_recent_sent_cache_from_disk() -> None:
        """热刷新跨进程写入的最近发送缓存，避免把其他任务发出的消息误判为对方消息。"""
        nonlocal self_sent_cache, self_sent_cache_mtime_ns
        try:
            current_mtime_ns = self_sent_file.stat().st_mtime_ns
        except Exception:
            current_mtime_ns = 0
        if current_mtime_ns <= 0 or current_mtime_ns == self_sent_cache_mtime_ns:
            return
        try:
            latest_cache = load_self_sent_cache(self_sent_file)
            removed = prune_self_sent_cache(latest_cache, time.time(), recent_sent_ttl_seconds)
            if removed:
                save_self_sent_cache(self_sent_file, latest_cache)
                try:
                    current_mtime_ns = self_sent_file.stat().st_mtime_ns
                except Exception:
                    pass
            self_sent_cache = latest_cache
            recent_sent_texts.clear()
            for cache_friend, items in self_sent_cache.items():
                recent_sent_texts[cache_friend] = collections.deque(items)
            self_sent_cache_mtime_ns = current_mtime_ns
        except Exception:
            return
    ...
    def looks_like_recent_self_sent(friend: str, text: str) -> bool:
        refresh_recent_sent_cache_from_disk()
        norm = _normalize_msg_text(text)
        if not norm:
            return True
    ...
    def find_chat_item_by_runtime_id(runtime_id: tuple[int, ...] | None) -> Any | None:
        if not runtime_id:
            return None
        try:
            chat_list = main_window.child_window(**lists.FriendChatList)
    ...
    def find_voice_convert_menu_item() -> Any | None:
        candidates = [
    ...
    def convert_voice_item_to_text(
        item: Any,
        timeout: float = 8.0,
        interval: float = 0.3,
        convert_item: Any | None = None,
    ) -> str:
        try:
            rid = tuple(item.element_info.runtime_id or ())
        except Exception:
    ...
    def extract_item_text_fallback(item: Any) -> str:
        try:
    ...
    def remember_self_sent_item(friend: str, item: Any, text: str = "") -> None:
        try:
            rid = tuple(item.element_info.runtime_id or ())
        except Exception:
            rid = ()
    ...
    def is_remembered_self_sent_item(friend: str, item: Any, text: str = "") -> bool:
        try:
            rid = tuple(item.element_info.runtime_id or ())
        except Exception:
            rid = ()
    ...
    def make_reply(friend: str, latest_text: str) -> str:
        ...
        try:
            return reply_service.make_reply(
                ReplyRequest(
                    wechat_id=wechat_id,
                    message=latest_text,
                ...
                fallback_reply=lambda: choose_reply(latest_text, args.reply, rules),
            )
        except ReplyServiceStop as exc:
            exc.emit(lambda text: print(text, flush=True))
    ...
    def _collect_item_anchor_rects(item: Any) -> list[tuple[int, int, int, int]]:
    ...
    def is_my_bubble_fast(item: Any, edit_area: Any) -> bool:
    ...
    def build_item_signature(item: Any) -> str:
    ...
    def build_peer_item_key(item: Any) -> str:
    ...
    def build_quote_probe_cache_key(item: Any, text: str = "") -> str:
        try:
            rid = tuple(item.element_info.runtime_id or ())
        except Exception:
            rid = ()
    ...
    def is_replyable_peer_item(item: Any, edit_area: Any) -> bool:
        """过滤非消息项与明显不可回复的占位项。"""
        try:
    ...
    def probe_item_has_quote_action(item: Any, text: str = "") -> bool:
    ...
    def is_peer_message_item(
        ...
        *,
        ...
        if not norm and not allow_empty_text:
            return False, ""
        # 优先用左右气泡落点排除“自己发的消息”。
        # 这类消息在 recent self-sent 缓存缺失时，仍可能混入未读补扫候选。
        if is_my_bubble_fast(item, edit_area):
    ...
    def _open_chat_history_window(source_chat: str) -> tuple[Any | None, str]:
    ...
    def _open_miniprogram_history_window(source_chat: str) -> tuple[Any | None, str]:
    ...
    def describe_main_window_unavailable(window: Any) -> str:
        if window is None:
            return "窗口对象为空"
        try:
    ...
    def is_main_window_unavailable(window: Any) -> bool:
    ...
    def handle_scan_exception(exc: Exception, context: str = "") -> None:
        try:
            reply_service.handle_no_subscriber_error(
                exc,
                wechat_id=str(self_wechat_id or "").strip(),
                context="扫描阶段",
                stop_code=461,
            )
        except ReplyServiceStop as stop_exc:
            stop_exc.emit(lambda text: print(text, flush=True))
            raise SystemExit(stop_exc.exit_code) from stop_exc
    ...
    def open_wechat_main_window_with_retry(max_attempts: int = 4, retry_delay: float = 1.0) -> Any:
